...
const loadHistorialAsistencia = () => {
    ...
    // Datos simulados iniciales (si no hay nada guardado)
    return [
        // La cédula '1301010101' es la usada por el estudiante de prueba 'José Loor'
...
// NUEVAS FUNCIONES PARA LA PERSISTENCIA DE JUSTIFICACIONES (COMPARTIDA CON ESTUDIANTE)
const loadJustificacionesGlobal = () => {
    ...
    // Datos simulados iniciales (Si no hay nada en localStorage, se añade este por defecto)
    return [
        { 
            cursoId: 'mat', 
            ...
            estado: 'Pendiente',
            adjuntoUrl: 'https://ejemplo.com/adjunto/cedula1301010101_20251015.pdf' // Simulando el adjunto
        },
    ];
};
...
const DOCENTE_DATA = {
    ...
    estudiantes: [
        // FIX: Cambiamos el nombre asociado a la cédula '1301010101' a 'José Loor'
        { cedula: '1301010101', nombre: 'José Loor', cursoId: 'mat', paralelo: 'A' },
        // Dejamos a Asriel con otra cédula o lo eliminamos para evitar confusiones
        ...
        // Si quieres mantener a Asriel Dreemurr, puedes asignarle una cédula nueva:
    ...
    justificaciones: loadJustificacionesGlobal(), // Ahora persistente y compartida
};
...
function saveAndAdvance() {
    ...
    // 1. Guardar la nueva asistencia en el historial local
    DOCENTE_DATA.historialAsistencia.push(...registroSesion);

    // 2. Guardar el historial actualizado en localStorage
    saveHistorialAsistencia(DOCENTE_DATA.historialAsistencia);
    
    // Solo se guarda la asistencia del paralelo actualmente seleccionado.
...
function generateReport() {
    ...
    if (estudiantesFiltrados.length === 0) {
        // Colspan ajustado a 5 (Cédula, Estudiante, Total Clases, Faltas, Acción)
    ...
    }).filter(data => data.totalClases > 0); // Ocultar estudiantes sin registros en el rango
    ...
    // Guardar el reporte en la variable global para ser guardado
    lastGeneratedReport = {
        nombre: reportTitle,
        cursoId, paralelo,
...
function saveReport() {
    ...
    // Requerir un nombre para guardar el reporte
    ...
    // 1. Agregar el reporte al array local
    DOCENTE_DATA.reportesGuardados.unshift(lastGeneratedReport);
    
    // 2. Guardar el array actualizado en localStorage
    saveReportesGuardados(DOCENTE_DATA.reportesGuardados);

    // Resetear la variable de reporte generado después de guardarlo
    lastGeneratedReport = null;
    document.getElementById('save-report-btn').style.display = 'none';
    
    // Recargar la vista si el modo es 'view'
...
function renderSavedReports(reports) {
    ...
    reports.forEach((report, index) => {
        ...
        // Asignar el índice como data-index para fácil acceso
...
function filterSavedReports() {
    ...
    const filteredReports = DOCENTE_DATA.reportesGuardados.filter(report => {
        ...
        // Filtrar por rango de fecha de GENERACIÓN
...
function viewSavedReportDetails(index) {
    ...
    // Mostrar el reporte como si fuera generado
    lastGeneratedReport = report;

    // Cambiar a la vista de "Generar" para usar la misma tabla de resultados
    document.getElementById('report-mode-select').value = 'generate';
    changeReportMode(); // Esto oculta save-report-btn

    // Rellenar los filtros (opcional, pero ayuda a la coherencia)
...
function loadJustificacionesPendientes(initialLoad) {
    // Cargar la lista más reciente desde el almacenamiento global
    ...
    if (initialLoad) return; // No renderizar en el load inicial (se hace al hacer clic en el tab)
    ...
    justificacionesPendientes.forEach(just => {
        const row = tableBody.insertRow();
        // ESTO ES LO QUE ARREGLAMOS: Buscar el nombre en la lista de estudiantes por la cédula
        ...
        // Lógica para mostrar el hipervínculo del adjunto
...
function aprobarJustificacion(fecha, cedula, cursoId) {
    // Buscamos la justificación por sus 3 identificadores únicos: fecha, cédula, y curso.
    ...
    if (index !== -1) {
        ...
        // Actualizar el historial de asistencia para reflejar la justificación
        ...
        // Guardar el listado de justificaciones actualizado
